# 小云雀短剧 Agent 智能预演体验指南

> 来源：https://xyq.jianying.com/tutorials/short-drama-agent-smart-preview
> 抓取时间：2026-09-23 11:20（离线快照，原文见链接）

[小云雀](/)/[教程中心](/tutorials)/智能预演浏览本页目录

1. [为什么要用智能预演](#JvzydkpVuokPmQxu8PKc3VD0nFb)
2. [功能简介](#MfMOdf7lFoUV6IxA16KcywsSnWe)
3. [功能详述](#JTGCdcGkooE3BOxZ45jcjc6wnUd)
   1. [如何使用？](#SiLRdJtnioMLyXxuu5lcLyIBnUd)
   2. [怎么修改？](#W3D0d0LkgoVLoNxWKqCcDd3dnYd)

小云雀 · 短剧 Agent

# 智能预演体验指南

预演整组镜头、统一规划分镜，并自动把关键帧插入对应片段，帮助多镜头剧情保持人物动作、运镜关系和空间逻辑一致。

## 为什么要用智能预演

****常见痛点****

1. 首尾帧只能解决两个镜头的一致性，多镜头仍需反复抽卡。
2. 运镜和空间逻辑容易跳变，正反打镜头难以保持一致。
3. 手动补救成本高，链路长且繁琐。

****推荐用法：****先把剧本和每幕分镜调整到位，一键生成智能预演；确认分镜图无误后，再生成视频。

## 功能简介

[![图片展示的是小云雀短剧Agent智能预演界面。画面中有一段文字描述，下方有“多选”和“智能预演”选项，其中“智能预演”被紫色勾选标识突出显示。下方有三个视频片段图标，分别是“片段01 8s”“片段02 12s”“片段03 10s”，每个片段图标右侧都有播放按钮。该图片与文档中介绍智能预演功能的内容相关，直观呈现了智能预演的操作界面及部分视频片段选项。](_files_小云雀短剧 Agent 智能预演体验指南/image_1.png)](https://p11-seeyou-cn.byteimg.com/tos-cn-i-e844mpvzdi/29eac4070e424dd8bbb39079a653745b~tplv-e844mpvzdi-compress:q90.image)

在片段轴中打开智能预演

****智能预演****基于镜头提示词，预演并统一规划关键分镜图，自动插入片段作为视频参考。

它相当于导演助理，先为整组镜头做好规划，从而降低抽卡率。

## 功能详述

### 如何使用？

[![这张图片呈现的是小云雀短剧Agent智能预演功能里的片段选择界面，界面顶部设有“多选”“智能分镜图”选项，鼠标光标正指向“智能分镜图”选项。界面内展示了5个时长不一的片段缩略图，从左至右依次为时长11s的片段01、9s的片段02、13s的片段03、9s的片段04、13s的片段05，其中时长11s的片段01被黑色边框选中，对应使用步骤里选中预演片段的操作，结合操作指南，该界面是选择智能预演对应片段的操作入口区域。](_files_小云雀短剧 Agent 智能预演体验指南/image_2.png)](https://p11-seeyou-cn.byteimg.com/tos-cn-i-e844mpvzdi/f55bedc4fc654a19ba2b86385df8efdf~tplv-e844mpvzdi-compress:q90.image)

选择一组需要预演的片段

[![这张图展示的是小云雀短剧Agent智能预演操作相关的分镜参考设置界面，界面上方标注当前为“片段10”，还提示了镜头调整、积分消耗相关信息。画面左侧有两个红色箭头，分别指向“分镜1的参考图”和“分镜2的参考图”的输入区域，明确了两段分镜的参考图来源，分别对应片段S8的分镜1、分镜2参考图。界面中还清晰标注了本片段的场景设定为梅菲尔纪念医院急诊区走廊白天全景，同时详细说明了两段分镜的时长、构图、人物角色及对应的动作、台词等内容。](_files_小云雀短剧 Agent 智能预演体验指南/image_3.png)](_files_小云雀短剧 Agent 智能预演体验指南/image_3.png)

查看自动插入的分镜参考图

1. 在分集编辑器底部点击「智能预演」，选择一组已经确认提示词的片段，再点击「生成智能预演」。
2. 生成后，分镜图会自动插入对应片段，并用绿点提示查看。
3. 确认分镜图无误后，直接生成视频。

****温馨提示****

1. 预演片段需要多于 2 个。
2. 所选片段引用的素材不能超过 9 个。

### 怎么修改？

[![图片展示的是小云雀短剧Agent智能预演界面。画面中呈现了“片段01”的分镜参考图，包含角色“米歇尔医生”和“艾莉丝·杜”等。下方有“智能预演”“智能生成”按钮，以及多个分镜图缩略图。左侧有角色、场景、素材、道具等分类选项。该图片与文档中“如何使用？”部分内容相关，直观呈现了智能预演时的界面布局及分镜参考图等关键信息。](_files_小云雀短剧 Agent 智能预演体验指南/image_4.png)](https://p11-seeyou-cn.byteimg.com/tos-cn-i-e844mpvzdi/f61b4babfcd54f4282c670bf18b91691~tplv-e844mpvzdi-compress:q90.image)

查看智能预演分镜结果

[![图片展示了小云雀短剧Agent智能预演界面。左侧是预演片段，上方有“美国游戏概念艺术风格”等信息，下方是“开挂500 - 分镜1 - 参考图”，画面中人物穿着蓝色上衣，手持物品。右侧是空白的预演画面，上方有“16:9”“1868”等信息。该图片与文档中“怎么修改”部分内容相关，用于说明在对整体不满意时，可选中片段进行再次抽卡，重新规划分镜内容的操作场景。](_files_小云雀短剧 Agent 智能预演体验指南/image_5.png)](https://p11-seeyou-cn.byteimg.com/tos-cn-i-e844mpvzdi/e37847e824bb46ebbc2913460b5ba7c4~tplv-e844mpvzdi-compress:q90.image)

重新抽卡规划片段分镜

****整体不满意：****选中片段再次抽卡，重新规划分镜内容。

****单个分镜图不满意：****

1. 打开分镜图右上角的编辑入口，进入画布。
2. 从分镜图拉出节点，输入修改想法并生成图片。
3. 把新图片添加到素材。
4. 在资产库中将新分镜图拖入编辑器。

****期待大家给我们多多反馈～****

继续阅读

- [****小云雀 Web 产品手册 →****从产品入门到完整创作流程，了解短剧与漫剧、视频与图片、营销内容、一镜到底和爆款复刻。](/tutorials/web-manual)
- [****小云雀 Seedance 2.5 使用手册 →****了解 Seedance 2.5 的模型能力、创作步骤、专属玩法与提示词，通过真实案例掌握视频创作。](/tutorials/seedance-2-5)
- [****小云雀 Seedance 2.0 使用手册 →****学习 Seedance 2.0 的沉浸式短片创作、素材引用、爆款复刻与模型切换，查看电影运镜、剧情、广告和知识科普案例及完整提示词。](/tutorials/seedance-2-0)
- [****小云雀 Seedream 5.0 Pro 用户手册 →****了解 Seedream 5.0 Pro 的精准编辑、多图融合、商业设计与艺术创作能力，查看参考图、提示词、使用技巧和能力边界。](/tutorials/seedream-5-0-pro)
- [****小云雀短剧 Agent 画布使用手册 →****学习短剧 Agent 资产创作画布的角色与场景节点、素材引用、资产库同步、节点编辑和快捷键，完成短剧资产整理与生成。](/tutorials/short-drama-agent-canvas)
- [****小云雀短剧 Agent 3D 导演台使用手册 →****从静态构图到动态预演，学习角色站位、动作与运镜、时间轴编排、导出参考，以及快捷键和按钮功能。](/tutorials/short-drama-agent-3d-director)
- [****小云雀创作 Agent 画布使用手册 →****学习创作 Agent 的对话与画布协作、资产引用、图片视频节点编辑、智能运镜、批量管理与创作记录分享。](/tutorials/creation-agent-canvas)
- [****小云雀营销 Agent 产品使用手册 →****从商品与卖点出发，学习营销 Agent 的 Showcase、创意库、Hook 库、风格库、画布和一键出海，查看营销视频案例。](/tutorials/marketing-agent)
- [****小云雀短剧 Agent 剧本助手 →****了解短剧 Agent 的热点库、专业剧本模型、对话共创、版本管理、小说改编、系列剧、解说剧和出海本土化能力。](/tutorials/short-drama-agent-script-assistant)
- [****短剧「重制转绘」使用手册 →****学习上传原片、设置画风和目标地区、解析与本土化资产、生成分集脚本、白模复刻、分镜精修和合成导出。](/tutorials/short-drama-remake)

内容来源：[小云雀短剧 Agent 智能预演体验指南](https://bytedance.larkoffice.com/wiki/C27ewQCAjiRm1wkiNt6cF3wXnhe)

[返回顶部 ↑](#content)
