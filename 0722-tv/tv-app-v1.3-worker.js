...
<head>
    ...
    <title>TV Monitor Wall · v1.3</title>
    <!-- Tailwind CSS (v3.4) -->
...
\`;
    ...
    function applyLayout(rebuild) {
        ...
        if (N === 0) { grid.style.width = '0'; grid.style.height = '0'; return; }
        ...
        const pxH = best.H_grid * W; // 注意 H_grid 是基于 W=1 的归一
        ...
        const targetShape  = best.partition.join(',');
        const needRebuild = rebuild || (currentShape !== targetShape) || (grid.dataset.count !== String(N));

        if (!needRebuild) {
            // 仅更新各 row 高度
            currentRows.forEach((row, i) => {
                const c = best.partition[i];
                row.style.height = (pxW / c * 9 / 16) + 'px';
            });
            return;
        }

        // 完全重建：先销毁所有活跃屏幕的 hls/timer（避免旧 failover 闭包继续跑）
        appState.activeScreens.forEach(id => destroyPlayer(id));
        grid.innerHTML = '';
        grid.dataset.count = String(N);
        const ids = [...appState.activeScreens].sort((a,b)=>a-b);
        let idx = 0;
        best.partition.forEach(c => {
        ...
        // 启动播放器
        appState.activeScreens.slice().sort((a,b)=>a-b).forEach(id => initPlayer(id));
        // v1.3: Re-bind user-pause tracking on newly created video elements
        if (rebuild) initUserPauseTracking();
    }
    ...
    function loadUrl(id, url, trySecure, srcGroup, lineIndex) {
        ...
        const handleFailover = (reason) => {
            ...
            if (next === 0) {
                ...
                if (st.cycleCount >= 3) {
                    // v1.3: 3 cycles failed — rotate to next channel
