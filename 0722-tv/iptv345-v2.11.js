// 345 - live stream proxy worker · v2.11
//
/*
 * ==================== EMBEDDED README ====================
 * Purpose: HLS proxy/adaptation layer for pre-existing upstreams. Production
 * blind deployment uses https://345.lishuhang.com/; consumer playlists must
 * not use the workers.dev hostname. No Worker, custom domain, DNS record,
 * workflow, schedule, or credential was created by v2.10.2.
 *
 * Route families:
 *   /<tid><id>.m3u8       345 dynamic source session resolver and proxy
 *   /ysp[cw]NN.m3u8       YSP URL injected by the existing refresh build
 *   /xuexi_<key>.m3u8     Xuexi dynamic URL injected by the refresh build
 *   /wso-<name>.m3u8      official-station fallback source proxy
 *   /health/<tid><id>     345 source health probe
 *   /                     full UI/status page (blind base emits text status)
 *
 * Operational scope: a successful HLS transport probe means that a manifest
 * and sampled media objects were retrievable. It does not guarantee DRM,
 * encryption, regional availability, or video/audio decoding results.
 * Existing GitHub Actions uses Playwright to refresh dynamic YSP/Xuexi values,
 * then builds and deploys the blind Worker. Treat refresh success, status
 * success, and transport success as separate signals.
 *
 * ==================== CHANGELOG v2.11 ====================
 * 2026-08-15:
 * - Add ten Live News Chat public HLS routes as /lnc1.m3u8 through
 *   /lnc10.m3u8. The Worker dynamically resolves the provider's public CDN
 *   node on demand, adds required public browser-context headers, and
 *   proxies/re-writes manifests, AES keys, and media resources.
 * - Blind stores only numeric IDs and non-display stream codes; this full
 *   artifact records the display-name-to-route mapping below.
 * - EarthTV was researched but is not included because no stable official
 *   page was discovered that can refresh the target stream token.
 *
 * ==================== CHANGELOG v2.10.2 ====================
 * 2026-08-15:
 * - Only the blind deployment artifact changed: its diagnostic health-check
 *   output no longer includes channel display names, preserving the blind
 *   artifact's non-descriptive status-page design.
 * - Full artifact functionality is unchanged from v2.10.1; this file only
 *   updates its version label and records the blind-only adjustment.
 *
 * ==================== CHANGELOG v2.10.1 ====================
 * 2026-08-15:
 * - Keep the historical encrypted-xac decoder and add a data-only fallback for
 *   current 345 play pages that expose decoder inputs as direct inline vars;
 *   upstream page JavaScript is never evaluated.
 * - The refresh builder now inserts xuexi routes before the generic
 *   /<tid><id>.m3u8 matcher, preventing hexadecimal xuexi keys from being
 *   misrouted into the 345 source decoder.
 * - Bump version marker to v2.10.1. No cloud entities were added.
 *
 * Known limits: some YSP tokens may still expire; per-channel 345 segments
 * can be intermittent; WSO fallbacks depend on upstream official endpoints.
 * External README and changelog are stored alongside this file.
 * ============================================================
 */
//
// Routes:
...
//   GET /lncNN.m3u8              Live News Chat numeric-route HLS proxy
//   GET /lncseg/<NN>/<resource>  Live News Chat media/key resource proxy
//
// v2.11 LNC full mapping (blind intentionally omits display names):
//   Bloomberg TV+ -> /lnc1.m3u8       CNBC -> /lnc2.m3u8
//   CNN International -> /lnc3.m3u8  CNN -> /lnc4.m3u8
//   DW News -> /lnc5.m3u8            Free Speech TV -> /lnc6.m3u8
//   Global News -> /lnc7.m3u8        MSNBC -> /lnc8.m3u8
//   NBC News Now -> /lnc9.m3u8       Sky News -> /lnc10.m3u8
//
// v2.10 changes (vs v2.9):
//   - Add wso-guizhou (贵州卫视) — api.gzstv.com API
//   - Add wso-jiangsu (江苏卫视) — jstv.com fixed URL (needs Referer)
//
...
// v2.11: Full artifact records display-name mapping; blind uses numeric IDs only.
const LNC_CHANNELS = {
  1: { name: 'Bloomberg TV+', slug: 'bloomberg_live' },
  2: { name: 'CNBC', slug: 'cnbc_live' },
  3: { name: 'CNN International', slug: 'cnni_live' },
  4: { name: 'CNN', slug: 'cnnus_live' },
  5: { name: 'DW News', slug: 'dw_live' },
  6: { name: 'Free Speech TV', slug: 'fstv_live' },
  7: { name: 'Global News', slug: 'global_hd' },
  8: { name: 'MSNBC', slug: 'msnbcintl_live' },
  9: { name: 'NBC News Now', slug: 'nbcnewsnow_live' },
  10: { name: 'Sky News', slug: 'skynews_live' },
};
const LNC_STREAMS = Object.fromEntries(Object.entries(LNC_CHANNELS).map(([id, entry]) => [id, entry.slug]));
const LNC_DIRECTORY_URL = 'https://data.lncoperations.ee/server.json';
const LNC_ROOT = 'https://livenewschat.eu/';
const LNC_NODE_TTL = 30 * 1000;
let lncNodeCache = { host: '', ts: 0 };
function lncHeaders() {
  return {
...
async function resolveLncHost(force = false) {
  if (!force && lncNodeCache.host && Date.now() - lncNodeCache.ts < LNC_NODE_TTL) return lncNodeCache.host;
  let response;
  try { response = await fetch(LNC_DIRECTORY_URL, { headers: { 'User-Agent': lncHeaders()['User-Agent'] } }); }
  catch (e) { return null; }
  if (!response.ok) return null;
  const text = await response.text();
  const match = text.match(/["']?best["']?\s*:\s*["']([a-z0-9-]+\.lncoperations\.ee)["']/i);
  if (!match) return null;
  const host = match[1].toLowerCase();
  if (!/^[a-z0-9-]+\.lncoperations\.ee$/.test(host)) return null;
  lncNodeCache = { host, ts: Date.now() };
  return host;
}
function validLncResource(resource) {
  return !!resource && !resource.startsWith('/') && !resource.startsWith('\\\\') && !/^https?:/i.test(resource) && !resource.includes('..') && !resource.includes('\\\\');
}
function rewriteLncManifest(manifest, routeId, request, host) {
  const origin = new URL(request.url).origin;
  const local = resource => origin + '/lncseg/' + routeId + '/' + host + '/' + encodeURIComponent(resource);
  return manifest.split('\n').map(line => {
    const trimmed = line.trim();
    if (!trimmed) return line;
    if (trimmed.startsWith('#')) return line.replace(/URI="([^"]+)"/g, (whole, resource) => validLncResource(resource) ? 'URI="' + local(resource) + '"' : whole);
    return validLncResource(trimmed) ? local(trimmed) : line;
  }).join('\n');
}
async function fetchLncUpstream(routeId, resource, retry = true) {
  const stream = LNC_STREAMS[routeId];
  if (!stream || !validLncResource(resource)) return null;
  let host = await resolveLncHost(false);
  if (!host) return null;
  let response;
  try { response = await fetch('https://' + host + '/hls/' + stream + '/' + resource, { headers: lncHeaders() }); }
  catch (e) { return null; }
  if (response.status === 403 && retry) {
    host = await resolveLncHost(true);
    if (!host) return response;
    try { return await fetch('https://' + host + '/hls/' + stream + '/' + resource, { headers: lncHeaders() }); }
    catch (e) { return null; }
  }
  return response;
}
async function fetchLncAtHost(host, routeId, resource) {
  const stream = LNC_STREAMS[routeId];
  if (!stream || !/^[a-z0-9-]+\.lncoperations\.ee$/.test(host) || !validLncResource(resource)) return null;
  try { return await fetch('https://' + host + '/hls/' + stream + '/' + resource, { headers: lncHeaders() }); }
  catch (e) { return null; }
}
async function handleLncM3u8(routeId, request) {
  if (!LNC_STREAMS[routeId]) return textResponse('Not found', 404);
  let host = await resolveLncHost(false);
  if (!host) return textResponse('LNC node resolution error', 502);
  let response = await fetchLncAtHost(host, routeId, 'index.m3u8');
  if (response && response.status === 403) {
    host = await resolveLncHost(true);
    response = host ? await fetchLncAtHost(host, routeId, 'index.m3u8') : null;
  }
  if (!response) return textResponse('LNC manifest fetch error', 502);
  if (!response.ok) return textResponse('LNC manifest failed: ' + response.status, 502);
  const manifest = await response.text();
  if (!manifest.includes('#EXTM3U')) return textResponse('LNC returned non-HLS response', 502);
  return new Response(rewriteLncManifest(manifest, routeId, request, host), {
    status: 200,
    headers: { 'Content-Type': 'application/vnd.apple.mpegurl', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'no-cache, no-store, must-revalidate', 'CDN-Cache-Control': 'no-store' },
  });
}
async function handleLncSeg(routeId, host, encodedResource) {
  let resource;
  try { resource = decodeURIComponent(encodedResource); }
  catch (e) { return textResponse('Invalid LNC resource encoding', 400); }
  if (!LNC_STREAMS[routeId] || !/^[a-z0-9-]+\.lncoperations\.ee$/.test(host) || !validLncResource(resource)) return textResponse('Invalid LNC resource', 400);
  const cache = caches.default;
  const cacheKey = new Request('https://lnc-cache.iptv345.local/' + routeId + '/' + host + '/' + encodedResource.slice(0, 160));
  const cached = await cache.match(cacheKey);
  if (cached) return new Response(cached.body, { status: 200, headers: { 'Content-Type': cached.headers.get('Content-Type') || 'application/octet-stream', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=600', 'X-Cache': 'HIT' } });
  const upstream = await fetchLncAtHost(host, routeId, resource);
  if (!upstream) return textResponse('LNC resource fetch error', 502);
  if (!upstream.ok) return textResponse('LNC resource failed: ' + upstream.status, 502);
  const body = await upstream.arrayBuffer();
  const contentType = upstream.headers.get('Content-Type') || 'application/octet-stream';
  const response = new Response(body, { status: 200, headers: { 'Content-Type': contentType, 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=600', 'X-Cache': 'MISS' } });
  try { cache.put(cacheKey, response.clone()).catch(() => {}); } catch (e) {}
  return response;
}
...
async function resolvePlayPhpUrl(tid, id) {
  ...
  // 3. Support both historical encrypted-xac pages and the current direct-variable page.
  // Old pages carry a long encrypted string which decrypts to xac. Current pages
  // expose the same decoder inputs directly in inline JavaScript. Parse only data
  // expressions; never evaluate source-page JavaScript.
  ...
  if (longVarVal) {
    try {
      ...
      if (xac.length >= 100 && xac.startsWith('<script>')) parsed = parseXac(xac);
    } catch (e) {}
  }
  if (!parsed || !parsed.keyValue || !parsed.staticToken || !parsed.dynamicToken || !parsed.xorSuffix) {
    try {
      parsed = parseXac(html);
    } catch (e) {
      throw new Error('failed to parse 345 decoder inputs: ' + e.message);
    }
  }
  if (!parsed.keyValue || !parsed.staticToken || !parsed.dynamicToken || !parsed.xorSuffix) {
    throw new Error('failed to extract required 345 decoder inputs');
  }

  // 4. Extract option value (線路1 = first <option>)
  const optM = html.match(/<option value="([^"]+)">[^<]*<\/option>/);
  if (!optM) throw new Error('option value not found in HTML');
  const optionValue = optM[1];

  // 5. Compute play.php URL
...
<header>
  <h1>📺 345</h1>
  <span class="subtitle">v2.10</span>
...
export default {
  async fetch(request, env, ctx) {
    ...
    // v2.11: /lnc<id>.m3u8 and /lncseg/<id>/<resource> (numeric LNC routes)
    const lncM3u8Match = path.match(/^\/lnc(\d+)\.m3u8$/);
    if (lncM3u8Match) {
      return handleLncM3u8(parseInt(lncM3u8Match[1], 10), request);
    }
    const lncSegMatch = path.match(/^\/lncseg\/(\d+)\/([a-z0-9-]+\.lncoperations\.ee)\/(.+)$/);
    if (lncSegMatch) {
      return handleLncSeg(parseInt(lncSegMatch[1], 10), lncSegMatch[2], lncSegMatch[3]);
    }
