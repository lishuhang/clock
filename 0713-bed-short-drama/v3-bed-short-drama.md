# 实测5个短剧App的首床率，34段录屏142个擦边片段-demo
...
本文基于对红果、拼多多、美团、快手、支付宝等5个主流短剧平台的信息流录屏统计。共采集34段不同时间段的录屏视频（其中原作者吴晓宇测试30段、书航测试4段），截至发稿已识别完成28段。按"同一部短剧的同一集连续出现合并为一个片段"的原则去重后，共获得1069个短剧片段记录，其中识别出142个含亲密戏镜头的片段、覆盖818部独立短剧。所有原始数据均通过多模态模型逐帧识别后人工核对。
...
上述的五个平台之中，拼多多、美团和快手等平台是人们印象中主要面向下沉市场的短剧平台。从已识别的28段录屏统计来看，**快手、红果和支付宝的平台有床率达到100%**——也就是说，每一次测试都必然出现亲密戏片段。拼多多和美团均为75%。

在床戏频率维度（亲密戏片段数 / 总片段数），拼多多以14.35%居首，意味着用户每刷约7个不同短剧片段就会遇到1个含擦边内容。美团14.01%、红果13.08%、支付宝13.62%紧随其后，快手11.05%。

最值得关注的是**首床位置**——用户多早会被"埋伏"。红果的首床位置均值仅为3.8，意味着用户平均刷到第4个不同短剧片段左右就会遇到第一个擦边片段。支付宝9.5、拼多多10.2、美团8.8、快手11.4。相比以"秒"为单位的统计，"片段位置"更贴近用户的真实"刷"体验——每一次滑动切换都意味着一个新片段。
...
从时段数据看，下午时段（14-18点）共采集10段录屏389个片段，亲密戏频率13.37%；深夜时段（22-23点）7段录屏308个片段，频率升至13.64%；晚间时段（19-21点）虽然只有1段录屏31个片段，频率9.68%。这印证了"深夜推送更激进"的行业观察——算法在用户更放松、更私密的时段，会放开擦边内容的闸门。

一些短剧的床戏并不发生在第一集，但系统还是会为用户推荐那些带有床戏的集数。比如《暖光倾落他心海》这部短剧，拼多多的推荐系统为小娱推荐的是第29集，恰好就是带有床戏和侵犯情节的集数。

![图5：宣传集偏差TOP5](v3-img5-styled.png)

本次数据中最极端的案例集中在支付宝平台——《爱情公寓3》第513集（偏差512）、旧梦不售·新景方长 第190集（偏差189）、《旧梦不值，新景方长》第190集（偏差189）、短剧·天降小福星，欢喜过大年 第174集（偏差173）等。这些案例都显示出算法主动定位"擦边集数"的行为模式——平台不仅知道哪一集有擦边，还主动把它推到用户面前，而非从第1集开始推送。
...
从数据来看，34段测试中快手、红果、支付宝100%命中亲密戏，142个擦边片段分布在5个平台、818部短剧中，最极端的推荐偏差达到512集——这些数字共同勾勒出一个"擦边常态化"的短剧生态。算法不仅知道哪一集有擦边，还会主动把擦边集数推到用户面前；平台不仅在白天放任擦边，还会在深夜加大推送力度。
