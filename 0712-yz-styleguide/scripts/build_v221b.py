...
═══════════════════════════════════════════════════════════════
  ...
  ── a→b 形态转换原则（v2.21b 核心规则）──
  当大图（a 形态）转 1:1 小图（b 形态）时，如内容密度过高，
  可灵活转换为更适合 1:1 的图表类型。原则：1 张 1:1 图只表达 1 个核心信息。
  常见转换路径（参考，不局限）：
  · 多维度堆叠对比（堆叠柱状图）→ 单维度饼图（每张 1 个对象）
  · 多序列折线对比 → 单序列哑铃图（每张 1 个维度）
  · 多行数据表 → 热力图（颜色编码替代数字密度）
  · 多卡 2×2 → 单卡 1 张（引言字号可大幅放大）
  转换规则：
  1. 一张 1:1 图只表达 1 个核心信息（"1 图 1 焦点"）
  2. 多维度对比拆为多张单维度图
  3. 转换后数据必须与原图一致
  4. 转换后字号必须满足最小字号下限
  5. 转换后内容必须铺满 1:1 画幅
  ...
  ── 组件复用原则（v2.21b 核心规则）──
  agent 不得重复造轮子。所有图表必须复用 SHARED CSS 类：
  · 阶段名+徽章：.phase-label / .phase-badge
  · 哑铃轨道：.dumbbell-track / .dumbbell-dot / .dumbbell-tag / .dumbbell-line
  · 坐标轴：.axis-scale
  · 图例：.chart-legend / .legend-item / .legend-swatch
  · 热力图：.hm-table / .hm-legend
  · 堆叠柱状图：.sb-wrap / .sb-row / .sb-bar / .sb-seg
  · 文字卡：.tc-grid / .tc-card / .tc-quote
  · 饼图：.pie-wrap / .pie-svg / .pie-legend-h
  · 多序列哑铃：.db5-wrap / .db5-sub / .db5-row
  · 散点：.sc6-wrap / .sc6-row
  新增图表类型时，优先扩展 SHARED CSS，不另立类名。
