...
═══════════════════════════════════════════════════════════════
  ...
  ── 1:1 画布规格 ──
  - 画布尺寸：900×900px（1:1）
  - 标题区（顶部，高度约120px = 画布1/7.5）：
    · 标题文字：宽度4/5（720px），字号34px（正文字号28×120%），字重heavy(900)
    · 行距1.35，最多折行4行
    · 右侧1/5（180px）：娱乐资本论图形logo（不带文字），高度约80px
  - 图表主体区（中部，flex:1，约660px高）：
    · 内容居中，4:3比例（如720×540）
    · 所有子图复用v2.21a的SHARED CSS类
  - 脚注区（底部，高度约120px）：
    · 灰色小字：字号18px（当前26×2/3≈17.3，取18）
    · 右下角不再带logo
  - 不带大型半透明浮水印（视频本身会加水印）

  ── 拆图规则 ──
  1. 按内容密度拆，让1:1画布在9:16竖屏居中显示时最小字体仍清晰可见
