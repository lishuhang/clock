#!/bin/bash
# Record all 17 animated HTMLs to MP4 using 3-step ffmpeg
ROUND3="/home/z/my-project/workspace-clock/0712-yz-styleguide/0811-test/round3"
FRAMES_BASE="/tmp/r3_frames"
...
record_one() {
    ...
    if [ -f "$MP4" ] && [ $(stat -c%s "$MP4") -gt 100000 ]; then
        echo "[$NAME] skip"
        return
    fi
    ...
    # Capture frames via Python (Playwright)
    python3 -c "
import asyncio, os
