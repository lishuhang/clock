// Cloudflare Worker - AI图片/视频生成器代理 v17
// v17: 画质档位 + 视频清晰度 + 媒体存储配额 + 邀请中心 + Grok固定尺寸映射
  ...
  <div class="main-area">
    <div class="left-panel">
      <div class="gen-form">
        ...
        <div class="form-group" id="qualityTierGroup" style="display:none">
          <div class="form-label-row">
            <label>画质档位</label>
            <span id="qualityTierHint" style="font-size:.72rem;color:var(--fg3)">1.5K·1024x1024·¥0.2/张</span>
          </div>
          <select id="qualityTierSelect" onchange="onQualityTierChange()">
        ...
        <div class="form-group" id="videoResolutionGroup" style="display:none">
          <div class="form-label-row">
            <label>视频清晰度</label>
            <span style="font-size:.72rem;color:var(--fg3)">上游 Grok 视频仅支持 480p 和 720p</span>
          </div>
          <select id="videoResolutionSelect" onchange="onVideoResolutionChange()">
            <option value="720p">720p</option>
            <option value="480p">480p</option>
          </select>
        </div>
        <div class="form-group" id="grokSizeHintGroup" style="display:none">
          <span id="grokSizeHintText" style="font-size:.72rem;color:var(--orange)"></span>
        </div>
        <div class="form-group">
...
<div class="modal-backdrop" id="settingsModal">
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div id="storageInfoContainer" style="margin-bottom:16px"></div>
      <div id="invitePanelContainer" style="display:none;margin-bottom:16px;padding:12px;border:1px solid var(--bg4);border-radius:var(--radius);background:var(--bg3)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
          <span style="font-size:.85rem;font-weight:600;color:var(--fg)">邀请好友</span>
          <button class="btn btn-xs btn-ghost" onclick="closeInvitePanel()">关闭</button>
        </div>
        <div style="font-size:.78rem;color:var(--fg2);margin-bottom:8px">好友通过链接注册后，双方均可获得平台额度奖励</div>
        <div style="display:flex;align-items:center;gap:8px">
          <input type="text" id="inviteLinkInput" readonly style="flex:1;padding:6px 10px;border-radius:6px;border:1px solid var(--bg4);background:var(--bg2);color:var(--fg);font-size:.8rem;font-family:var(--mono)">
          <button class="btn btn-sm" onclick="copyInviteLink()" style="border-color:var(--accent);color:var(--accent2)">复制链接</button>
        </div>
      </div>
...
<div class="help-panel" id="helpPanel">
  ...
  <div class="help-panel-body">
    ...
    <dl class="changelog">
      <dt>v17 (当前)</dt>
      <dd>新增画质档位：gpt-image-2 支持 1.5K/2.5K/4K 三档分辨率，自动计算尺寸并显示价格提示</dd>
      <dd>新增视频清晰度：grok-imagine-video 支持 480p/720p 选择</dd>
      <dd>新增媒体存储配额：显示每个账号的存储用量，超过阈值支持一键清理旧媒体</dd>
      <dd>新增邀请中心：可查看邀请链接、复制分享，邀请好友双方获额度奖励</dd>
      <dd>Grok 图片模型固定 5 种合法上游尺寸映射，lite 模型固定 784x1168</dd>
      <dd>新增链式邀请注册：新注册账号自动使用上一个账号的邀请码，形成账号n邀请账号n+1的链式关系</dd>
      <dd>Worker 后端新增 /account/storage/cleanup 和 /account/invite 代理路由</dd>
...
function isGrokImgModel(m){return m==='grok-imagine-image'||m==='grok-imagine-image-pro'||m==='grok-imagine-image-lite'}
...
document.querySelectorAll('.ratio-btn').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.ratio-btn').forEach(x=>x.classList.remove('active'));b.classList.add('active');selectedRatio=b.dataset.ratio;var model=document.getElementById('modelSelect').value;if(model==='gpt-image-2'){updateQualityTierHint()}updateGrokSizeHint()}));
...
// ===== 初始化 =====
loadSettingsUI();markInterruptedTasks();restoreLiveImages();renderAll();checkNewDay();renderFullHistory();onModelChange();
