// Cloudflare Worker - AI图片/视频生成器代理 v14
// v14: 新增视频生成(grok-imagine-video) + 更多宽高比(14种) + 数量1-6连续 + 视频时长选择 + 参考图支持视频 + 视频/图片代理
...
<div class="app">
  <div class="topbar">
    ...
    <div class="topbar-right">
      ...
      <span>总额度: <span id="totalCreditsTop" class="topbar-credits">0</span></span>
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div style="border-top:1px solid var(--bg4);padding-top:16px">
        <div class="form-group">
          <label>用户名前缀</label>
          <input type="text" id="usernamePrefix" value="ml" placeholder="ml">
        </div>
        <div class="form-group">
          <label>默认密码</label>
...
function defaultState(){return{accounts:[],settings:{usernamePrefix:'ml',defaultPassword:'Ml@2026Proxy',rotationStrategy:'most-credits',autoCheckin:true,autoRegister:true,theme:'system'},activeAccountIndex:-1,rotationIndex:0,lastAutoDay:''}}
...
function generateUsername(){const p=state.settings.usernamePrefix||'ml';const c='abcdefghijklmnopqrstuvwxyz0123456789';let s='';for(let i=0;i<8;i++)s+=c[Math.floor(Math.random()*c.length)];return p+'_'+s+'_'+Date.now().toString(36)}
function generatePassword(){return state.settings.defaultPassword||'Ml@2026Proxy'}
...
async function registerAccount(){
  ...
  try{
    ...
    if(r.ok){const d=await r.json();const a={username:u,password:pw,sessionToken:'',credits:d.user?.imageCredits||3,lastCheckinDay:'',lastCheckinTs:0,createdAt:Date.now(),disabled:false,userId:d.user?.id||''};state.accounts.push(a);if(state.activeAccountIndex<0)state.activeAccountIndex=0;saveState();renderAll();toast('直接注册成功: '+u,'success');try{await loginAccount(state.accounts.length-1)}catch(e){}return state.accounts[state.accounts.length-1]}
  ...
  for(let attempt=0;attempt<maxRetries;attempt++){
    try{
      ...
      const a={username:u,password:pw,sessionToken:st,credits:d.user?.imageCredits||3,lastCheckinDay:'',lastCheckinTs:0,createdAt:Date.now(),disabled:false,userId:d.user?.id||''};
...
// ===== 登录/签到/额度 =====
async function loginAccount(i){const a=state.accounts[i];if(!a)return;try{const r=await apiFetch('/auth/login',{method:'POST',body:JSON.stringify({username:a.username,password:a.password})});const d=await r.json();if(!r.ok)throw new Error(d.error||'登录失败');a.sessionToken=r.headers.get('X-Session-Token')||'';a.credits=d.user?.imageCredits||a.credits;a.lastCheckinDay=d.user?.lastCheckInDay||a.lastCheckinDay;a.userId=d.user?.id||a.userId;saveState();renderAll();return d}catch(e){console.warn('登录'+a.username+'失败:',e.message);throw e}}
...
function renderTopbar(){const total=state.accounts.filter(a=>!a.disabled).reduce((s,a)=>s+(a.credits||0),0);document.getElementById('totalCreditsTop').textContent=total}
...
function renderAccountsTable(){const today=new Date().toISOString().split('T')[0],c=document.getElementById('accountsTableContainer');if(!state.accounts.length){c.innerHTML='<p style="color:var(--fg3);text-align:center;padding:20px">暂无账号</p>';return}c.innerHTML='<table class="accounts-table"><thead><tr><th>用户名</th><th>额度</th><th>签到</th><th>操作</th></tr></thead><tbody>'+state.accounts.map((a,i)=>'<tr><td style="font-family:var(--mono);font-size:.8rem;'+(a.disabled?'opacity:.5;text-decoration:line-through':'')+'">'+escHtml(a.username)+'</td><td>'+a.credits+'</td><td>'+(a.lastCheckinDay===today?'<span style="color:var(--green)">\\u2713</span>':'<span style="color:var(--orange)">\\u2717</span>')+'</td><td><button class="btn btn-sm" onclick="loginAndRefresh('+i+')">登录</button> <button class="btn btn-sm" onclick="checkinAccount('+i+')" '+(a.lastCheckinDay===today?'disabled':'')+'>签到</button> <button class="btn btn-sm btn-danger" onclick="removeAccount('+i+')">删除</button></td></tr>').join('')+'</tbody></table>'}
...
function saveSettings(){state.settings.usernamePrefix=document.getElementById('usernamePrefix').value.trim()||'ml';state.settings.defaultPassword=document.getElementById('defaultPassword').value.trim()||'Ml@2026Proxy';state.settings.rotationStrategy=document.getElementById('rotationStrategy').value;state.settings.autoCheckin=document.getElementById('autoCheckin').checked;state.settings.autoRegister=document.getElementById('autoRegisterChk').checked;saveState();toast('设置已保存','success')}
function loadSettingsUI(){document.getElementById('usernamePrefix').value=state.settings.usernamePrefix||'ml';document.getElementById('defaultPassword').value=state.settings.defaultPassword||'Ml@2026Proxy';document.getElementById('rotationStrategy').value=state.settings.rotationStrategy||'most-credits';document.getElementById('autoCheckin').checked=state.settings.autoCheckin!==false;document.getElementById('autoRegisterChk').checked=state.settings.autoRegister!==false;applyTheme()}
function exportAccounts(){const d=state.accounts.map(a=>({username:a.username,password:a.password}));const b=new Blob([JSON.stringify(d,null,2)],{type:'application/json'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download='ai_image_generator_accounts_'+new Date().toISOString().split('T')[0]+'.json';a.click();URL.revokeObjectURL(u);toast('已导出','success')}
function importAccounts(){document.getElementById('importFileInput').click()}
async function handleImportFile(input){const file=input.files[0];if(!file)return;try{const txt=await file.text();const data=JSON.parse(txt);const arr=Array.isArray(data)?data:[data];let n=0;arr.forEach(item=>{if(item.username&&item.password&&!state.accounts.some(a=>a.username===item.username)){state.accounts.push({username:item.username,password:item.password,sessionToken:'',credits:0,lastCheckinDay:'',lastCheckinTs:0,createdAt:Date.now(),disabled:false,userId:''});n++}});if(n>0){if(state.activeAccountIndex<0)state.activeAccountIndex=0;saveState();renderAll();toast('导入'+n+'个账号，正在登录...','success');for(let i=0;i<state.accounts.length;i++){if(!state.accounts[i].sessionToken&&!state.accounts[i].disabled){try{await loginAccount(i);await sleep(300)}catch(e){}}}}else{toast('无新账号可导入','info')}}catch(e){toast('导入失败: '+e.message,'error')}input.value=''}
