// Cloudflare Worker - AI生图 v24
// v24: 修复会话过期提示"请先登录"(自动重新登录并重试) + 批量签到按钮适配上游已下线签到功能(友好提示) + 新增图片生成完成浏览器通知(设置页可开关)
...
<nav id="topNav">
  ...
  <div class="nav-right">
    <span class="points">剩余 <span id="usableCreditsTop">0</span></span>
...
<div id="mainWrap">
  <div id="leftPanel">
    ...
    <div class="extra-btns">
      ...
      <button class="btn btn-outline" disabled title="功能开发中">去除SynthID</button>
    </div>
  </div>
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div style="display:flex;gap:6px;margin-bottom:16px">
        <button class="btn btn-sm btn-outline" onclick="exportAccounts()">导出账号</button>
...
async function registerAccount(){
  ...
  try{
    const r=await fetch('https://grok.17nas.com/local-api/auth/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.assign({username:u,password:pw},inviteCode?{inviteCode:inviteCode}:{})),credentials:'omit'});
...
// 排除已耗尽账号的账号选择
function selectAccountWithExclude(excludeSet){const avail=state.accounts.map((a,i)=>({...a,_i:i})).filter(a=>!a.disabled&&a.credits>0&&a.sessionToken&&!excludeSet.has(a.username));if(!avail.length)return null;switch(state.settings.rotationStrategy){case'most-credits':avail.sort((a,b)=>b.credits-a.credits);return avail[0];case'round-robin':state.rotationIndex=state.rotationIndex%avail.length;const p=avail[state.rotationIndex];state.rotationIndex++;saveState();return p;case'newest':avail.sort((a,b)=>b.createdAt-a.createdAt);return avail[0];default:return avail[0]}}
...
async function executeTask(entry,model,prompt,size,hasRef,currentRefImages,isVideo,duration,qualityTier,resolutionName){
  ...
  while(retryCount<=maxRetries){
    ...
    try{
      ...
      if(!acc){entry.status='error';entry.error='账户余额不足，请先签到、充值或填写自己的 Bearer Token';entry.progressText='失败';updateHistory(entry.id,{status:entry.status,error:entry.error,progressText:entry.progressText});toast(entry.error,'error');notifyGenerationComplete(entry);return}
      ...
      const d=await r.json();if(!r.ok){const errMsg=d.error||d.message||'创建任务失败';
        ...
        // 额度不足：标记当前账号，自动换号重试
        if(errMsg.includes('insufficient_quota')||errMsg.includes('额度不足')){
          exhaustedAccounts.add(acc.username);
          var ai2=state.accounts.findIndex(a=>a.username===acc.username);
          if(ai2>=0){state.accounts[ai2].credits=0;saveState();renderAll()}
          toast(acc.username+' 额度不足，自动换号中...','info');
          // v24: 上游已下线签到功能，不再调用 checkinAccount；改为直接换号
          retryCount++;entry.progressText='额度不足，换号重试 ('+retryCount+'/'+maxRetries+')';entry.progress=0;
...
function extractErrorMessage(raw){var msg=raw||'未知错误';if(msg.includes('上游图片接口返回失败')||msg.includes('上游')&&msg.includes('失败'))return msg;if(msg.includes('临时下架')||msg.includes('暂不开放'))return'视频模型已临时下架，暂不可用';if(msg.includes('not an image model'))return'视频模型API端点错误，请更新版本';if(msg.includes('content_policy')||msg.includes('policy')||msg.includes('safety'))return'内容违反政策(OpenAI安全策略限制)';if(msg.includes('rate_limit'))return'请求频率超限，请稍后再试';if(msg.includes('insufficient_quota')||msg.includes('额度不足'))return'平台额度不足，请先签到或充值';if(msg.includes('超时')||msg.includes('timeout'))return msg;if(msg.includes('已注册'))return'当前IP今日已注册过账号';if(msg.includes('无效')||msg.includes('invalid'))return'请求参数无效: '+msg;if(msg.includes('failed'))return'生成失败: '+msg;return msg}
...
function updateConcurrencyUI(){const mx=maxConcurrency(),cur=activeSlots;var el1=document.getElementById('concurrencyInfo');if(el1)el1.textContent='并发: '+cur+'/'+mx;var el2=document.getElementById('concurrencyDetail');if(el2)el2.textContent='可用账号'+state.accounts.filter(a=>!a.disabled&&a.credits>0&&a.sessionToken).length+'个 x '+CONCURRENT_PER_ACCOUNT+' = '+mx+'并发槽位'}
