...
async function testCustomApiKey(){
  ...
  try{
    ...
    var d=await r.json();
    if(!r.ok){
      var err=d.error||d.message||d.detail||('HTTP '+r.status);
    ...
    else if(pd.status==='failed'){toast('Key 有效但生成失败: '+(pd.error||'未知'),'error')}
    else{toast('Key 有效，任务状态: '+pd.status+' (progress: '+(pd.progress||'N/A')+')','info')}
  }catch(e){
    toast('Key 测试异常: '+e.message,'error');
  }
}
// v24: 通知开关交互
