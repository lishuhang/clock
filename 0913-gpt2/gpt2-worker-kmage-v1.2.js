// ============================================================
// gpt2-worker-kmage-v1.2.js —— 版本: kmage-kdr-1.2
...
============================================================ -->
<title>AI生图 · kmage-kdr-1.2</title>
...
.brand h1{font-size:18px;margin:0;letter-spacing:.5px;white-space:nowrap}
.brand .ver{font-size:11px;color:var(--text-muted);align-self:center;white-space:nowrap}
.chan-sel{padding:4px 8px;border:1px solid var(--border);border-radius:8px;background:#fbfcfe;color:var(--text);font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;outline:none;margin-left:2px}
...
.icon-btn{width:34px;height:30px;padding:0;display:inline-flex;align-items:center;justify-content:center;border-radius:8px;border:1px solid var(--border);background:#fff;color:var(--text-2);cursor:pointer;transition:all .15s}
...
.hist-list li .chtag{font-size:10px;color:var(--text-muted);border:1px solid var(--border);border-radius:4px;padding:0 4px}
...
.gift-line{display:flex;align-items:center;gap:8px;margin-top:2px}
...
.tl .td{flex:none;width:86px;font-family:ui-monospace,Menlo,Consolas,monospace;font-size:11.5px;color:var(--accent-dark);font-weight:600;padding-top:1px}
...
<div class="wrap">
  <header class="top">
    <div class="brand">
      <svg class="logo" viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" aria-hidden="true"><title>AI生图</title><circle cx="10" cy="19" r="6"/><path d="M16 13v12"/><path d="M24 15v10"/><path d="M24 8.5v.01"/></svg>
      <h1>AI生图</h1>
      <span class="ver">kmage-kdr-1.2</span>
    ...
    <div class="top-right">
      ...
      <button class="icon-btn" id="poolBtn" type="button" title="设置" aria-label="设置"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3.2"/><path d="M12 2.8v2.4M12 18.8v2.4M21.2 12h-2.4M5.2 12H2.8M18.4 5.6l-1.7 1.7M7.3 16.7l-1.7 1.7M18.4 18.4l-1.7-1.7M7.3 7.3L5.6 5.6"/></svg></button>
      <button class="icon-btn" id="logBtn" type="button" title="控制台" aria-label="控制台"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2.8" y="4.2" width="18.4" height="15.6" rx="2.6"/><path d="M7 9.5l3.2 2.8L7 15.1"/><path d="M12.8 15.5H17"/></svg></button>
      <button class="icon-btn" id="docsBtn" type="button" title="关于" aria-label="关于"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9.2"/><path d="M12 11v5.4"/><path d="M12 7.6v.01"/></svg></button>
    </div>
  </header>
...
async function importPoolData(txt){
  ...
  if(data&&data.type==='ai-image-settings'){
    ...
    renderSettings();renderGiftLine();applyChannel();renderAll();
...
async function generate(){
  ...
  var histEntry={ts:Date.now(),run:currentRunId,channel:channel,prompt:(prompt||'（参考图创作）').slice(0,80),model:$('model').value,size:channel==='kdr'?((($('kdrSize')&&$('kdrSize').value)||'1K')+' / '+$('ratio').value):$('ratio').value,quality:$('quality').value,account:'',status:'running',error:''};
  ...
  try{
    ...
    saveState();saveKdrState();saveLogs();generating=false;btn.disabled=false;
...
function renderHist(){
  ...
  for(var i=0;i<histItems.length;i++){
    ...
    html+='<li><span>'+(h.status==='ok'?'<span class="okc">✓</span>':(h.status==='running'?'⋯':'<span class="errc">✗</span>'))+'</span>'
      ...
      +'<span>'+esc(String(h.model||'').replace('gpt-image-',''))+'</span>'
...
function renderAll(){renderBadge();if($('poolDialog').open){renderPool();renderGiftLine()}}
...
var LOGO_SVG='<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" aria-hidden="true"><circle cx="10" cy="19" r="6"/><path d="M16 13v12"/><path d="M24 15v10"/><path d="M24 8.5v.01"/></svg>';
function ABOUT_HTML(){
  var h=[];
  ...
  h.push('<p style="color:var(--accent-dark);font-weight:600">AI Agent 提示：本页为自包含单文件 Cloudflare Worker（后端代理+内嵌前端+号池），可将本页整体作为 skill 调用；本文档即调试入口与使用说明。服务自描述: GET /about（含真实上游地址）；健康检查: GET /healthz。</p>');
  h.push('<h3>1. 这是什么</h3>');
  h.push('<p>本页把「准备凭据 → 发起生图 → 取回结果」整条链路自动化，提供 kmage（默认）与 kdr 两个免费生图通道，可通过 header 上的通道选择器随时切换。生成结果、运行日志与设置全部保存在你自己的浏览器本地。</p>');
  ...
  h.push('<p>· 「导出全部设置」：一份 JSON 打包 kmage 号池与全部设置项 + kdr 自定义 Key 与共享 Key 缓存 + 通知开关 + 当前通道，用于跨浏览器/跨设备完整迁移。</p>');
  h.push('<p>· 「导入设置」自动识别格式（设置包 / 旧版号池 / 裸账号数组）；缺会话的账号导入后自动重登恢复。导出文件含明文凭据，请妥善保管。</p>');
  h.push('<h3>6. 运行控制台</h3>');
  ...
  h.push('<h3>8. 数据模型（localStorage）</h3>');
  h.push('<pre>kmage_state_v1   accounts[]: {email,password,session,apiKey,apiKeyId,apiKeyHint,'+String.fromCharCode(10)+'                 credits,lastCheckinDay,eligibleAt,createdAt,disabled,lastError}'+String.fromCharCode(10)+'                 abandoned[]: 已删除/归档账号'+String.fromCharCode(10)+'                 settings: {rotationStrategy,autoRegister,autoCheckin,'+String.fromCharCode(10)+'                 emailDomain,notificationsEnabled}'+String.fromCharCode(10)+'kdr_state_v1     customKeys[]: 自定义付费 Key（可选）'+String.fromCharCode(10)+'                 gift: {key,alias,ts} 共享 Gift Key 缓存（10 分钟）'+String.fromCharCode(10)+'kmage_channel_v1 当前通道（kmage | kdr，默认 kmage）'+String.fromCharCode(10)+'kmage_logs_v1    运行日志环形缓冲（最近 300 条，detail 截断 300 字符）</pre>');
  h.push('<h3>9. 反模式化原则（沿袭马良 v27.2）</h3>');
  ...
  h.push('<div class="ti"><span class="td">2026-09-13</span><span class="tc"><b>kmage-kdr-1.2（本版）</b>：kdr 通道修复复活（适配 key/host 入 body 新契约、任务轮询、结果图代理转存，免费 Gift Key 实测可出图）；header 通道选择器（默认 kmage）；UI 重构——footer 移除，「号池」→「设置」、「日志」→「控制台」、「文档」+「帮助」合并为「关于」，均改线条 SVG 图标；新增画笔颜料盘 favicon 与「ai」字母组合 logo（header 与关于页顶部）；设置级 JSON 导入导出（号池+全部设置项）；对外文档源站脱敏（以「kmage 站点 / kdr 站点」表述）；本文档整合马良至今完整时间线。</span></div>');
  h.push('</div>');
  h.push('<h3>12. 边界说明</h3>');
...
// ---------- 启动 ----------
(async function init(){
  loadState();loadKdrState();loadChannel();loadLogs();migrateLegacyKdrKeys();
  renderSettings();renderGiftLine();renderBadge();renderHist();renderRefs();bindAll();bindPoolActions();
  applyChannel();
  appLog('页面加载完成（'+VERSION+'，通道 '+channel+'，日志 '+(runLogs.length)+' 条已恢复）','i');
