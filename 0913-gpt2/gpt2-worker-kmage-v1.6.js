// ============================================================
// gpt2-worker-kmage-v1.6.js —— 版本: kmage-kdr-1.6
...
//
// 版本: kmage-kdr-1.6 (2026-09-23)
//   v1.6 变更（净号直注 / Client-born Accounts + 403 自愈）:
//   - 背景: v1.5 上线后真机实测 kmage 直连生图仍 403 环境异常——被选中的
//     号池账号全部是 0913 经 Worker 从数据中心 IP 注册的「代理出身号」,
//     实测确认上游风控按生图请求出口 IP 判定（/v1 与 /api/tasks 双端点、
//     API Key 与会话 Cookie 双鉴权、任意 Origin/UA/XFF 均同拒，数据中心
//     出口一律 403），且注册接口无 CORS/Origin 校验、text/plain 请求体
//     照常解析（实测 201）——访客浏览器可直连注册
//   - 修复: 注册环境与使用环境一致——registerAccount 改为浏览器直连
//     注册优先（no-cors 简单请求 + text/plain JSON 体，无预检；失败自动
//     回退代理注册），注册后经 Worker 会话代理登录/建 Key（会话类不受
//     风控影响），生图仍浏览器直连——新号「住宅 IP 出生 + 住宅 IP 使用」
//   - 403 account_environment_abnormal 自愈: 标记账号 envFlagAt（24h 内
//     选号跳过）→ 自动换号重试 → 若仍被拒且开启自动注册则浏览器直连
//     注册净号再试；全部失败时透出诚实错误与替代建议
//   - 号池 UI: 账号状态列增加「环境异常」「直生」标记；日志修复
//     非 JSON 上游错误响应被误标「非JSON响应」的显示问题
    ...
    return jsonResp({
      ...
      channels: {
        kmage: {
          ...
          mode: '账号号池（注册/签到攒积分）+ OpenAI 兼容生图；v1.5 直连优先；v1.6 净号直注（浏览器直连注册，注册环境=使用环境）+ 403 环境异常自愈',
          endpoints: {
            session_proxy: '/api/kmage/* -> ' + UPSTREAM + '/api/*（会话经 X-Kmage-Session 头；Cookie 会话无法跨域，仍走代理）',
            register_direct: '浏览器直连 ' + UPSTREAM + '/api/auth/register（no-cors 简单请求 + text/plain 体解析 JSON；实测无 CORS/Origin 校验；v1.6 起注册环境与使用环境一致），网络失败回退 session_proxy',
            v1_proxy: '/kmage/v1/* -> ' + UPSTREAM + '/v1/*（Bearer 透传；v1.5 起为直连失败时的自动回退）',
            v1_direct: '浏览器直连 ' + UPSTREAM + '/v1/images/generations（Bearer；v1.5 起生图直连优先——2026-09-23 上游将数据中心出口 IP 的生图请求全部判为环境异常，Worker 出口 IP 已被拦截；v1.6 实测代理出身号住宅直连仍拒，净号直注+403 自愈）'
          },
      ...
      shared: { logs_storage: 'localStorage: kmage_logs_v1', channel_storage: 'localStorage: kmage_channel_v1', form_storage: 'localStorage: kmage_form_v1（创作面板选项记忆，模型按通道分存）', history_storage: 'localStorage: kmage_hist_v1（任务历史，成功/失败均入册）', direct_mode: 'v1.5 直连优先: 生图类请求由浏览器直连上游（本接口即地址来源），网络失败自动回退 Worker 代理；kmage 会话类仍走代理（Cookie 跨域不可携带）；v1.6: kmage 注册亦直连优先（净号出生），403 环境异常自动标记/换号/直注重试' },
...
<!-- ============================================================
  ...
  - v1.6 净号直注: 实测代理出身号（数据中心 IP 注册）即使住宅 IP 直连
    生图仍被拒；注册改为浏览器直连优先（no-cors 简单请求，注册环境=
    使用环境）；403 环境异常自动标记/换号/直注净号重试（自愈链）
...
============================================================ -->
<title>AI生图 · kmage-kdr-1.6</title>
...
<div class="wrap">
  <header class="top">
    <div class="brand">
      ...
      <div class="brand-txt">
        <h1>AI生图</h1>
        <span class="ver">kmage-kdr-1.6</span>
      </div>
...
<div class="dlg-body" id="poolBody">
  ...
  <div class="settings-grid">
    ...
    <div>
      <div class="chk"><input type="checkbox" id="autoRegisterChk" checked><label for="autoRegisterChk">无号可用或全被环境风控时自动直连注册净号（+1 分）</label></div>
    </div>
    <div>
...
async function kmageV1Via(url,opts,apiKey,modeLabel,dispPath){
  ...
  appLog('[kmage] '+(opts.method||'GET')+' '+dispPath+'（'+modeLabel+'）→ HTTP '+resp.status+' ('+dur+'ms)',
    ...
    resp.ok?'':((data&&data.error)?JSON.stringify(data):(rawTxt?('非JSON响应: '+rawTxt):JSON.stringify(data).slice(0,240))));
...
// v1.6 净号直注: 浏览器直连注册（no-cors 简单请求）。
// 实测依据（2026-09-23）: 上游注册端点无 CORS 预检处理（OPTIONS 405）也无
// Origin 校验，且不校验 Content-Type——text/plain 请求体照常按 JSON 解析
// （curl 实测 201），因此浏览器可用 no-cors 简单请求（不触发预检）直连注册，
// 使新号出生环境 = 访客住宅 IP + 真实浏览器指纹，与后续直连生图环境一致。
// 响应为 opaque（不可读），注册是否成功由随后的 Worker 代理登录验证。
async function registerDirect(email,password){
  await fetch(directBases.kmage+'/api/auth/register',{
...
async function registerAccount(silent){
  ...
  var a={id:'acc_'+randStr(10),email:email,password:password,session:'',apiKey:'',apiKeyId:'',apiKeyHint:'',
    credits:1,lastCheckinDay:'',eligibleAt:0,createdAt:Date.now(),disabled:false,lastError:'',envFlagAt:0,bornDirect:false};
  var directOk=false;
  if(directBases.kmage){
    appLog('[kmage] 注册新账号 '+maskEmail(email)+'（浏览器直连注册: 出生环境=使用环境）','i');
    try{await registerDirect(email,password);directOk=true}
    catch(e){appLog('[kmage] 直连注册请求失败（'+(e.message||e)+'），回退代理注册（数据中心出生，存在被风控标记风险）','w')}
  }else{
    appLog('[kmage] 注册新账号 '+maskEmail(email)+'（代理注册: 直连地址未就绪）','i');
  }
  if(directOk){
    // 直连注册响应不可读，用 Worker 代理登录验证并取得会话
    try{await loginAccount(a);a.bornDirect=true}
    catch(e){directOk=false;appLog('[kmage] 直连注册后登录验证失败（'+(e.message||e)+'），回退代理注册','w')}
  }
  if(!directOk){
    var r=await kmageApi('/auth/register',{method:'POST',body:JSON.stringify({email:email,password:password})},a);
    if(!r.ok){
      // 直连注册可能实际已成功（响应不可读），先验证登录再判失败
      var lg=false;
      try{await loginAccount(a);lg=true}catch(e2){}
      if(!lg){
        appLog('[kmage] 注册失败 '+maskEmail(email),'e','','');
        throw new Error('注册失败: '+upstreamErrMsg(r,'HTTP '+r.status));
      }
      appLog('[kmage] 代理注册报错但登录验证通过（直连注册已生效），继续','w');
    }else if(!a.session){
      await loginAccount(a); // 极少数情况上游未返回 Set-Cookie，主动登录补会话
    }
  }
  a.credits=1;
  try{var me=await withSession(a,function(){return kmageApi('/auth/me',{},a)});if(me.ok&&me.data&&me.data.user&&typeof me.data.user.credits==='number')a.credits=me.data.user.credits}catch(e){}
  // 注册成功后立刻创建 API Key（密钥只显示一次，必须当场保存）
  try{await createKeyFor(a)}catch(e){a.lastError='建Key失败:'+e.message}
  appLog('[kmage] 注册完成 '+maskEmail(email)+'（'+a.credits+' 分'+(a.bornDirect?'，浏览器直连出生':'，代理出生')+'，24h 后可签到）','s');
...
// v1.6: 账号 24h 内被上游环境风控拒绝（403 account_environment_abnormal）标记
function envFlagged(a){return !!(a&&a.envFlagAt&&(Date.now()-a.envFlagAt)<24*3600*1000)}
// v1.6: 上游环境风控响应识别（403 + code account_environment_abnormal，或文案含「环境异常」）
function isEnvAbnormalResp(r){
  if(!r||r.status!==403)return false;
  var e=(r.data&&r.data.error)||{};
  if(e.code==='account_environment_abnormal')return true;
  var m=(typeof e.message==='string')?e.message:'';
  if(m.indexOf('环境异常')>=0)return true;
  var raw=(r.data&&r.data._raw)?String(r.data._raw):'';
  return raw.indexOf('account_environment_abnormal')>=0||raw.indexOf('环境异常')>=0;
}
function pickAccount(){
  var cands=[];
  for(var i=0;i<state.accounts.length;i++){
    var a=state.accounts[i];
    if(!a.disabled&&a.apiKey&&(typeof a.credits==='number'&&a.credits>=1)&&!envFlagged(a))cands.push(a);
  }
...
async function findReadyAccount(){
  ...
  // 2) 有会话但没 Key → 补建（v1.6: 跳过 24h 内被环境风控标记的号）
  for(var i=0;i<state.accounts.length;i++){
    var x=state.accounts[i];
    if(!x.disabled&&!envFlagged(x)&&x.session&&!x.apiKey&&(typeof x.credits==='number'&&x.credits>=1)){
      try{await createKeyFor(x);saveState();renderAll();return x}catch(e){}
    }
  }
  // 3) 有 Key 但额度显示 0/未知 → 刷新额度再试（v1.6: 跳过环境风控标记号）
  for(var j=0;j<state.accounts.length;j++){
    var y=state.accounts[j];
    if(!y.disabled&&!envFlagged(y)&&y.apiKey){
...
async function generateKmage(histEntry,runId,t0){
  ...
  if(acc.createdAt&&(Date.now()-acc.createdAt)<24*3600*1000){
    appLog('[kmage] 提示: 该账号注册未满 24 小时。24h 冷却只限制签到，不影响生图；若本号生图异常将自动换号/直连注册净号重试','w','',runId);
  }
  ...
  if(r.ok&&r.data&&r.data.data&&r.data.data.length&&r.data.data[0].b64_json){
    ...
    }else if(r.status===403&&isEnvAbnormalResp(r)){
      // v1.6 自愈链: 上游环境风控按生图出口 IP 判定（实测 /v1 与 /api/tasks 双端点、
      // API Key 与会话 Cookie 双鉴权同拒；代理出身号即使住宅 IP 直连仍会被拒）。
      // ① 标记该账号（24h 内选号跳过）；② 换其他未标记账号重试一次；
      // ③ 仍被拒且开启自动注册 → 浏览器直连注册净号（出生环境=使用环境）再试；
      // ④ 全部失败 → 诚实透出错误原文与建议，不夸不瞒
      appLog('[kmage] 上游环境风控拒绝该账号（403 环境异常），进入自愈链','w','',runId);
      setStatus('genStatus','上游环境风控拒绝该账号，正在换号 / 尝试浏览器直连注册净号…','info');
      acc.envFlagAt=Date.now();acc.lastError='环境异常 403';saveState();renderAll();
      var lastR=r;var lastAcc=acc;
      var accE=null;
      for(var ei=0;ei<state.accounts.length;ei++){
        var ce=state.accounts[ei];
        if(ce.id!==acc.id&&!ce.disabled&&ce.apiKey&&(typeof ce.credits==='number'&&ce.credits>=1)&&!envFlagged(ce)){accE=ce;break}
      }
      if(accE){
        histEntry.account=maskEmail(accE.email);
        appLog('[kmage] 自愈①: 切换到 '+maskEmail(accE.email)+'（积分 '+accE.credits+'）','i','',runId);
        setStatus('genStatus','使用 '+maskEmail(accE.email)+'（'+accE.credits+' 分）· 重试…','info');
        lastR=await genWithAccount(accE,body,runId);
        if(lastR.ok&&lastR.data&&lastR.data.data&&lastR.data.data.length&&lastR.data.data[0].b64_json){
          finishGenSuccess(lastR,accE,histEntry,runId);return;
        }
        lastAcc=accE;
        if(lastR.status===403&&isEnvAbnormalResp(lastR)){
          lastAcc.envFlagAt=Date.now();lastAcc.lastError='环境异常 403';saveState();renderAll();
          appLog('[kmage] 换号后仍被环境风控拒绝，标记 '+maskEmail(lastAcc.email),'w','',runId);
        }else{
          throw new Error(upstreamErrMsg(lastR,'换号重试后仍失败'));
        }
      }else{
        appLog('[kmage] 自愈①: 号池内没有可换的未标记账号','w','',runId);
      }
      if(state.settings.autoRegister){
        var na=null;
        try{na=await registerAccount(true)}catch(e){appLog('[kmage] 自愈②: 净号注册失败: '+(e.message||e),'w','',runId)}
        if(na&&na.apiKey){
          histEntry.account=maskEmail(na.email);
          appLog('[kmage] 自愈②: 净号 '+maskEmail(na.email)+' 就绪（'+(na.bornDirect?'浏览器直连出生':'代理出生')+'），重试生图','i','',runId);
          setStatus('genStatus','净号 '+maskEmail(na.email)+' 重试生图…','info');
          lastR=await genWithAccount(na,body,runId);
          if(lastR.ok&&lastR.data&&lastR.data.data&&lastR.data.data.length&&lastR.data.data[0].b64_json){
            finishGenSuccess(lastR,na,histEntry,runId);return;
          }
          lastAcc=na;
        }
      }else{
        appLog('[kmage] 自愈②: 自动注册未开启（设置内可开启），跳过净号直注','w','',runId);
      }
      var tail=(lastAcc&&lastAcc.bornDirect&&lastR&&lastR.status===403&&isEnvAbnormalResp(lastR))
        ?'；净号（浏览器直连出生）亦被拒——当前网络出口或上游风控策略可能整体收紧，可更换网络后重试、稍后再试，或改用 kdr 通道'
        :'（可尝试: 设置内开启自动注册以直连注册净号、更换网络后重试、或改用 kdr 通道）';
      throw new Error(upstreamErrMsg(lastR,'生图失败')+tail);
    }else{
...
function acctStatus(a){
  ...
  if(envFlagged(a))return '<span class="pill warn" title="'+new Date(a.envFlagAt).toLocaleString()+' 生图被上游环境风控拒绝（403），24h 内选号自动跳过；过期自动恢复">环境异常</span>'+(a.bornDirect?'<span class="pill ok" title="浏览器直连注册（出生环境=使用环境）">直生</span>':'')+(a.lastCheckinDay===todayGmt8()?'<span class="pill ok">今日已签</span>':'');
  if(a.lastCheckinDay===todayGmt8())return '<span class="pill ok">今日已签</span>'+(a.bornDirect?' <span class="pill ok" title="浏览器直连注册（出生环境=使用环境）">直生</span>':'');
  var now=Date.now();
  if(a.eligibleAt&&a.eligibleAt>now){
    return '<span class="pill warn">待激活 '+fmtRemain(a.eligibleAt-now)+'</span>'+(a.bornDirect?' <span class="pill ok" title="浏览器直连注册（出生环境=使用环境）">直生</span>':'');
  }
  if(typeof a.credits==='number'&&a.credits<=0)return '<span class="pill err">0分</span>'+(a.bornDirect?' <span class="pill ok" title="浏览器直连注册（出生环境=使用环境）">直生</span>':'');
  return '<span class="pill ok">正常</span>'+(a.bornDirect?' <span class="pill ok" title="浏览器直连注册（出生环境=使用环境）">直生</span>':'');
}
function renderPool(){
  var tb=$('poolTbody');
...
function ABOUT_HTML(){
  ...
  h.push('<p><b>kmage 通道（默认）</b>：基于「kmage 站点」官方 OpenAI 兼容 API。自动注册账号（v1.6 起浏览器直连注册，出生环境=使用环境）→ 每日签到攒积分 → API Key 调用生图；1 积分 = 1 张图，失败的请求上游自动返还积分；遇到 403 环境异常会自动标记账号、换号并直连注册净号重试。支持 3 个模型、7 种比例、精细度与图生图（≤10 张参考图）。</p>');
  ...
  h.push('<p>· 注意：24 小时冷却只限制「签到」，不限制「生图」——新注册的账号凭 1 积分可尝试立即生图。若生图被上游以「环境异常」拒绝（风控按出口 IP 与账号出身判定），工具会自动标记该账号（24 小时内选号跳过）、换号并浏览器直连注册净号重试。</p>');
  ...
  h.push('<pre>/            页面（HTML+CSS+JS 全内嵌，无外部资源）'+String.fromCharCode(10)+'/healthz     健康检查（版本/上游/通道/时间）'+String.fromCharCode(10)+'/about       服务自描述 JSON（AI Agent 排障入口，含真实上游，v1.5 直连地址来源）'+String.fromCharCode(10)+'/api/kmage/* -> kmage 上游 /api/*   会话类代理（登录/签到/Key/额度；v1.6 起注册改为浏览器直连优先，此处为回退）'+String.fromCharCode(10)+'/kmage/v1/*  -> kmage 上游 /v1/*    Bearer 透传（v1.5 起为直连失败时的自动回退）'+String.fromCharCode(10)+'/api/kdr/*   -> kdr 上游 /api/*     透明转发（v1.5 起为直连失败时的自动回退）'+String.fromCharCode(10)+'/kdr/img     kdr 结果图拉取代理（v1.5 起为直连图床失败时的回退）'+String.fromCharCode(10)+'v1.5 直连优先: 生图类请求由浏览器直连上游（住宅 IP + 真实 UA，地址自 /about 获取），网络失败自动回退本 Worker 代理；2026-09-23 起上游拉黑数据中心出口 IP，代理路径仅作兜底'+String.fromCharCode(10)+'v1.6 净号直注: 注册亦由浏览器直连上游（no-cors 简单请求，出生环境=使用环境）；403 环境异常自动标记账号/换号/直注净号重试（自愈链）</pre>');
  ...
  h.push('<div class="ti"><span class="td">2026-09-23</span><span class="tc"><b>kmage-kdr-1.6（本版）</b>：净号直注 + 403 自愈——v1.5 上线后真机实测 kmage 直连生图仍报「账号使用环境异常」：被选中的号池账号是 0913 经 Worker 从数据中心 IP 注册的「代理出身号」，实测确认风控按生图请求出口 IP 判定（/v1 与 /api/tasks 双端点、API Key 与会话 Cookie 双鉴权、任意 Origin/UA/XFF 均同拒，数据中心出口一律 403）；同时实测注册端点无 CORS/Origin 校验且 text/plain 体照常按 JSON 解析（201），访客浏览器可直连注册。v1.6 将注册改为浏览器直连优先（no-cors 简单请求，出生环境=使用环境，网络失败回退代理注册，注册结果由 Worker 代理登录验证），403 account_environment_abnormal 触发自愈链：标记账号（24h 选号跳过）→ 换未标记号重试 → 浏览器直连注册净号再试 → 全部失败时透出错误原文与替代建议；号池状态列新增「环境异常」「直生」标记；修复非 JSON 错误响应在日志中被误标「非JSON响应」的显示问题。</span></div>');
  h.push('<div class="ti"><span class="td">2026-09-23</span><span class="tc"><b>kmage-kdr-1.5</b>：直连优先修复双通道风控拦截——2026-09-23 起 kdr 上游将 Cloudflare Worker 出口 IP 段列入免费 Key 黑名单（生成报「此 IP 已被加入免费 Key 黑名单」），kmage 上游将生图环境风控扩大到所有数据中心出口 IP（报「账号使用环境异常，充值后解锁」），两通道经 Worker 代理的生图全部被拒；实测两上游均开放 CORS（Access-Control-Allow-Origin: *），故 v1.5 将生图类请求改由访客浏览器直连上游（住宅 IP + 真实 UA，即上游期望的正常使用环境），Worker 代理降级为直连网络失败时的自动回退；直连地址运行时经 GET /about 自描述接口获取（脱敏原则不变，上游域名不写入页面静态文本）；kmage 会话类操作（Cookie 会话，无法跨域携带）不受风控影响仍走代理；kdr 结果图优先浏览器直连图床（实测不校验 Referer）失败回退 /kdr/img 代理；控制台日志对每次请求标注「直连/代理」路径与回退事件。</span></div>');
  h.push('</div>');
