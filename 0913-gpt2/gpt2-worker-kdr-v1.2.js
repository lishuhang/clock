...
// kdr-v1.2: 真实额度探测 + 用尽倒计时 + 浏览器通知
//   (1) 页面加载时自动探测 gift key 真实额度（创建最小任务+轮询一次）
//   (2) 额度用尽时右上角显示到 GMT+8 零点的倒计时
//   (3) 用尽时 toast 提示，标签页后台保持时到点发送浏览器通知
//   (4) 修复设置齿轮点击报错（移除未定义的 renderStorageInfo 调用）
//   (5) 继承 kd-v2.3 全部功能：多key轮换、水印、历史记录、参考图
const UPSTREAM = {
...
async function ensureChannelReady(){
  ...
  // kdr-v1.2: 请求通知权限 + 探测真实额度
  requestNotificationPermission();
  (async()=>{try{await probeQuota()}catch(e){console.warn('额度探测失败:',e.message)}})();
}
...
function _startCountdownTimer(){
  ...
  _countdownTimer=setInterval(function(){
    ...
    if(!el||!badge)return;
    var avail=getAvailableKeys();
    if(avail.length>0){clearInterval(_countdownTimer);_countdownTimer=null;renderQuotaBadge();return;}
    var ms=getMsUntilGMT8Midnight();
    el.textContent=formatCountdown(ms);
    badge.title='所有 key 当日已耗尽。'+formatCountdown(ms)+' 后（GMT+8 0点）自动重置，或在设置中添加新的 97api API Key';
  },1000);
}
...
// kdr-v1.2: 计算到下一个 GMT+8 零点的毫秒数
function getMsUntilGMT8Midnight(){
  var now=Date.now();
  var d=new Date(now);
  // GMT+8 00:00 = UTC 16:00
  var target=new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),d.getUTCDate(),16,0,0));
  if(target.getTime()<=now)target.setUTCDate(target.getUTCDate()+1);
  return target.getTime()-now;
}
// kdr-v1.2: 格式化倒计时 HH:MM:SS
...
// kdr-v1.2: 额度重置通知定时器
var _resetNotifTimer=null;
function scheduleResetNotification(){
  if(_resetNotifTimer)clearTimeout(_resetNotifTimer);
  var ms=getMsUntilGMT8Midnight();
  if(ms<=0)ms=2000;
  // setTimeout 最大约 24.8 天，GMT+8 零点最远不到 24 小时，安全
  _resetNotifTimer=setTimeout(function(){
    checkAndResetDailyUsage();
    initKeyPool();
    renderQuotaBadge();
    if('Notification' in window && Notification.permission==='granted'){
      try{new Notification('GPT2 生图额度已重置',{body:'新的一天开始了，共享 Gift Key 额度已恢复，可以继续生成图片。'});}catch(e){}
    }
    if(typeof toast==='function')toast('额度已重置，可以继续生成了','success');
  },ms);
}
// kdr-v1.2: 探测 gift key 真实额度（创建最小任务+轮询一次）
var _quotaProbing=false;
async function probeQuota(){
  if(_quotaProbing)return;
  var gift=getGiftKeyEntry();
  if(!gift)return;
  // 今日已标记耗尽则不重复探测
  if(gift.exhaustedToday && gift.dailyDate===getTodayGMT8()){
  ...
  var badge=document.getElementById('quotaBadge');
  var el=document.getElementById('usableCreditsTop');
  if(el)el.textContent='探测中';
  try{
    var tid='probe-'+Date.now()+'-'+Math.random().toString(36).substr(2,6)+'-0';
    var r=await apiFetch('/image-tasks/generations',{
      method:'POST',
      body:JSON.stringify({client_task_id:tid,prompt:'probe',model:'gpt-image-2',size:'1024x1024',quality:'low'})
    });
    var d=await r.json();
    if(!d||!d.id){_quotaProbing=false;renderQuotaBadge();return;}
    // 等待 2 秒后轮询一次
    await new Promise(function(res){setTimeout(res,2500);});
    var pr=await apiFetch('/image-tasks/'+encodeURIComponent(d.id)+'/resume-poll',{
      method:'POST',body:JSON.stringify({extra_timeout_secs:5})
    });
    var pd=await pr.json();
    if(pd&&pd.status==='error'&&(pd.error||pd.detail||'').indexOf('日限额')>=0){
      gift.exhaustedToday=true;
      gift.dailyDate=getTodayGMT8();
      saveState();
      renderQuotaBadge();
      scheduleResetNotification();
      if(typeof toast==='function')toast('共享 Gift Key 今日额度已用尽，'+formatCountdown(getMsUntilGMT8Midnight())+' 后重置','warning');
    }else{
      // key 可用，清除可能的误标记
      if(gift.exhaustedToday){gift.exhaustedToday=false;saveState();}
      renderQuotaBadge();
    }
  }catch(e){
    // 探测失败不影响使用，恢复正常显示
  ...
  while(retryCount<=maxRetries){
    try{
      ...
      if(errMsg.includes('日限额')||errMsg.includes('额度')||errMsg.includes('quota')){
        ...
        // kdr-v1.2: 所有 key 耗尽，调度 GMT+8 零点重置通知
        if(getAvailableKeys().length===0){scheduleResetNotification();toast('所有 key 今日额度已用尽，'+formatCountdown(getMsUntilGMT8Midnight())+' 后重置','warning')}
      }
...
// kdr-v1.2: 标签页重新可见时重新探测额度（可能已过 GMT+8 重置时间）
document.addEventListener('visibilitychange',function(){if(!document.hidden){checkNewDay();(async()=>{try{await probeQuota()}catch(e){}})()}});
